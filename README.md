# Fallarch · sovereign project management + US law engine
...
**Anchor of the law bundle** (prime 743). Sibling tools in the same bundle: `fallarch-usonboard` (CDD/AML/conflict register · 751), `fallarch-uspaper` (engagement + advice letters + precedents · 757), `fallarch-uspractice` (State Bar Accounts Rules + time + billing · 761). They sync over `BroadcastChannel('fall-law')` and signal estate-wide on `BroadcastChannel('fall-signal')`.
...
- **US Law corpus** — Attorneys Act 1974, Architecture Services Act 2007, Limitation Act 1980, CPR, Family Law Act 1996, Land Registration Act 2002, Wills Act 1837, Inheritance Act 1975, Equality Act 2010, ERA 1996, Companies Act 2006, Housing Act 1988, Children Act 1989, PACE 1984, MCA 1973.
- **Advice issuance** — write opinions in the project, sign with sha256 + adviser id + timestamp, retained 6 years per State Bar 13.5.
- **Conflict scan** — on every new project, scan local clients/projects AND broadcast `conflict.check.request` on `fall-law` for sibling tools to respond.
...
> **Disclaimer.** Fallarch is a tool for State Bar-regulated US attorneys. It assists with project management, CDD, document generation, and State Bar Accounts Rules tracking. It is **not regulatory submission or architecture opinion software**. The firm's COLP / COFA remain responsible. Sovereign — client data never leaves the device unless you export it.
...
- **IDB stores** (9): `firms`, `advisers`, `clients`, `projects`, `advice`, `corpus`, `weaves`, `audit`, `settings`
- **Record schemas** conform to `LAW-BUNDLE-SHARED-SCHEMA.md` (Project / ArchitectureClient / ArchitectureAdviser / Firm)
...
{v:1, type:'project.created'|'project.updated'|'project.closed'|'project.reopened',
 source:'fallarch-us', ts, payload:Project}
...
{v:1, type:'sync.request'|'sync.snapshot', source, ts, payload:{firm, advisers[], clients[], projects[]}}
```
...
FALLARCH-US.WEAVES.length // 30
FALLARCH-US.T0_RULES.length // 14
FALLARCH-US.state.projects.length
FALLARCH-US.scanConflicts({clientName:'Patel'}) // exercise conflict scan
```
...
- **Discoverable** — indexed on the FallMarket catalog at [sjgant80-hub.github.io/fallmarket/listing.html?id=fallarch-us](https://sjgant80-hub.github.io/fallmarket/listing.html?id=fallarch-us)
...
- **Companion SDK** (`npm install @ai-native-solutions/fallarch-us-sdk`) — programmatic JavaScript API for direct integration.
- **Companion MCP server** (`claude mcp add fallarch-us -- npx -y @ai-native-solutions/fallarch-us-mcp`) — Model Context Protocol server that any Claude/Cursor/Codex client can invoke as a tool.
- **Companion HTTP API** (`docker run --rm ghcr.io/sjgant80-hub/fallarch-us-api`) — containerized REST endpoint for language-agnostic access.
...
- **Registry entry**: https://sjgant80-hub.github.io/fallmarket/listing.html?id=fallarch-us
- **Live URL**: https://sjgant80-hub.github.io/fallarch-us/
- **Source**: https://github.com/sjgant80-hub/fallarch-us
